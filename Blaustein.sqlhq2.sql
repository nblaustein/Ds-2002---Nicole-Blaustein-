...
CREATE TABLE `stocks_db`.`company_info` (
   `company_name` varchar(45) NOT NULL,
   `stock_ticker` varchar(45) DEFAULT NULL,
   `industry` varchar(45) DEFAULT NULL,
   PRIMARY KEY (`company_name`));
 CREATE TABLE `stocks_db`.`current_portfolio` (
   `stock_ticker` varchar(45) NOT NULL,
   `num_shares` int NOT NULL,
   `date_purchased` date DEFAULT NULL,
   `price_purchased` int DEFAULT NULL,
   `current_price` int DEFAULT NULL,
   PRIMARY KEY (`num_shares`));
